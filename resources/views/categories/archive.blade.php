            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                ...
                <div class="text-center text-md-start">
                    <h3 class="fw-bold mb-1 text-dark">
                        أرشيف التصنيفات <i class="fa-solid fa-box-archive text-warning ms-2"></i>
                    </h3>
                    <p class="text-muted mb-0 small">إدارة واستعادة أو حذف التصنيفات المؤرشفة نهائياً</p>
                </div>

                <!-- Action Buttons (اليسار) -->
                <div class="d-flex gap-2 flex-wrap justify-content-center align-items-center">
                    
                    <!-- 🗑️ زر تفريغ الأرشيف بالكامل -->
                    @if($categories->count() > 0)
                        <form action="{{ route('categories.forceDeleteAllArchive') }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-custom shadow-sm" onclick="return confirm('تحذير شديد: سيتم حذف جميع التصنيفات في الأرشيف نهائياً ولا يمكن التراجع عن هذه العملية! هل أنت متأكد؟')">
                                <i class="fa-solid fa-trash-arrow-up me-1"></i> تفريغ الأرشيف بالكامل
                            </button>
                        </form>
                    @endif

                    <a href="{{ route('categories.index') }}" class="btn btn-primary btn-custom shadow-sm" style="background-color: #7c3aed; border: none;">
                        <i class="fa-solid fa-layer-group me-1"></i> إدارة التصنيفات النشطة
                    </a>
                ...
                <table class="table table-hover mb-0 text-center align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-muted small">{{ $category->deleted_at->format('Y-m-d H:i') }}</td>
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <!-- زر الاستعادة -->
                                    <form action="{{ route('categories.restore', $category->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button class="btn btn-sm btn-outline-success btn-custom" title="استعادة" onclick="return confirm('هل تريد استعادة هذا التصنيف؟')">
                                    ...
                                    <form action="{{ route('categories.forceDelete', $category->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-sm btn-outline-danger btn-custom" title="حذف نهائي" onclick="return confirm('تحذير: سيتم حذف التصنيف نهائياً ولا يمكن التراجع! هل أنت متأكد؟')">

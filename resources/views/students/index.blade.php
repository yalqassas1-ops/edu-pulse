...
<head>
    ...
    <title>إدارة الطلاب - EduPulse</title>
    ...
    <style>
        ...
        .badge-gender { background-color: #e0e7ff; color: #3730a3; font-weight: 600; padding: 6px 12px; border-radius: 20px; }
    </style>
</head>
<body class="p-3 p-md-5">
    ...
    <div class="container">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div>
                    <h3 class="fw-bold mb-1 text-dark">
                        <i class="fa-solid fa-user-graduate text-primary me-2"></i> إدارة الطلاب
                    </h3>
                    <p class="text-muted mb-0 small">عرض وتعديل بيانات الطلاب المسجلين</p>
                </div>
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('students.create') }}" class="btn btn-primary btn-custom shadow-sm d-inline-flex align-items-center">
                        <i class="fa-solid fa-user-plus me-2"></i> إضافة طالب جديد
                    </a>
                    <a href="{{ route('students.archive') }}" class="btn btn-warning text-dark btn-custom shadow-sm d-inline-flex align-items-center fw-bold">
                        <i class="fa-solid fa-box-archive me-2"></i> أرشيف الطلاب
                    </a>
        ...
        <!-- Excel Actions Section (التصدير والاستيراد) -->
        <div class="card card-custom p-3 mb-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <!-- زر تصدير Excel -->
                <div>
                    <a href="{{ route('students.export', ['search' => request('search')]) }}" class="btn btn-success btn-custom shadow-sm d-inline-flex align-items-center">
                        <i class="fa-solid fa-file-excel me-2"></i> تصدير البيانات إلى Excel
                    </a>
                </div>

                <!-- نموذج استيراد Excel -->
                <form action="{{ route('students.import') }}" method="POST" enctype="multipart/form-data" class="d-flex align-items-center gap-2 m-0">
                    @csrf
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-start fw-bold text-dark">
                                <i class="fa-solid fa-user me-2 text-secondary"></i>{{ $student->name }}
                            </td>
                            <td class="text-muted">{{ $student->email }}</td>
                            <td class="text-muted dir-ltr">{{ $student->phone ?? 'غير مدخل' }}</td>
                            <td>
                            ...
                            <td>
                                @if($student->avatar)
                                    <a href="{{ Storage::url($student->avatar) }}" target="_blank" class="btn btn-sm btn-outline-info btn-custom">
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('students.edit', $student->id) }}" class="btn btn-sm btn-outline-warning btn-custom" title="تعديل">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('students.destroy', $student->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-sm btn-outline-danger btn-custom" title="نقل إلى الأرشيف" onclick="return confirm('هل أنت تأكد من نقل هذا الطالب إلى الأرشيف؟')">

...
<head>
    ...
    <title>تعديل بيانات التسجيل - EduPulse</title>
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    <h2 class="fw-bold m-0 text-dark">
                        تعديل بيانات التسجيل <i class="fa-solid fa-pen-to-square text-warning ms-1"></i>
                    </h2>
                    <small class="text-muted">تحديث معلومات التسجيل رقم #{{ $enrollment->id }}</small>
                </div>
                <div>
            ...
            <form action="{{ route('enrollments.update', $enrollment->id) }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="enrollment_date" id="enrollment_date" class="form-control" 
                               value="{{ old('enrollment_date', \Carbon\Carbon::parse($enrollment->enrollment_date ?? $enrollment->enrolled_at)->format('Y-m-d')) }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-select" required>
                            <option value="active" {{ old('status', $enrollment->status) == 'active' ? 'selected' : '' }}>نشط (Active)</option>
                            <option value="completed" {{ old('status', $enrollment->status) == 'completed' ? 'selected' : '' }}>مكتمل (Completed)</option>
                            <option value="cancelled" {{ old('status', $enrollment->status) == 'cancelled' ? 'selected' : '' }}>ملغى (Cancelled)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-warning fw-bold px-4">
                        <i class="fa-solid fa-check me-1"></i> تحديث البيانات
                    </button>
                </div>
            </form>
        </div>
